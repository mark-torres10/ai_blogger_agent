...
export async function POST(request: NextRequest) {
  try {
    ...
    const { topic, customTopic } = await request.json();
    ...
    // Read the personalized style guide
    const styleGuide = `# Personalized Writing Style Guide - Mark Torres
...
Write THREE-FOUR paragraphs about "${actualTopic}" in Mark's authentic voice. The content should be ${contentType} in nature and follow these specific requirements:
...
3. **Structure**: Exactly two paragraphs with natural flow between them
...
8. **Self-Deprecating Humor**: Include at least one instance of self-deprecating humor
